...
/*  START Mojo Web Services MODIFICATIONS */

$rci_mobileBypass = false;//default state

    //echo 'In mdetect mobile bypass:' . $_REQUEST['mobileBypass'] .'<br>'	;	    
    //IF - bypass parameter set, react
if (
    isset($_REQUEST['mobileBypass'])
    && in_array($_REQUEST['mobileBypass'], array('true', 'false'))
) {
    //IF bypass param set to bypass, set a session cookie and bypass during session
    if ('true' === $_REQUEST['mobileBypass']) {
        $rci_mobileBypass = true;        
        setcookie ('rci_mobileBypass', 'true', 0);
    } else if ('false' === $_REQUEST['mobileBypass']) {
        //param sent to remove bypass. Delete cookie
        setcookie ('rci_mobileBypass', '', time() - 3600);
    }
    
    //ELSEIF - cookie is set instructing bypass
} else if (
	    isset($_COOKIE['rci_mobileBypass'])
	    && 'true' === $_COOKIE['rci_mobileBypass']
	) {
	$rci_mobileBypass = true;    
}

if ($rci_mobileBypass === true) {
    //bypassing redirects
} else {
    //Have been given the go ahead for redirecting, check for mobile request
    $mobileDetection = new uagent_info();
  
  
  
  
 
    if ($mobileDetection->DetectMobileLong()) {
